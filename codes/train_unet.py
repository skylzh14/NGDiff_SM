...
select_spectral = []
spe = 16
channel = 1 #3d channel = 1  3d channel = 16


epochs = 500
lr = 1e-4
#只用于提取特征，不用于生成任务，而在低噪声影响下的特征更代表数据的分布，所以选取的时间步可以少一点
T=6
...
def train():
    dataloader = HSIDataLoader(
        {"data":{"data_sign":"512_512", "padding":False, "batch_size":batch_size, "patch_size":patch_size, "select_spectral":select_spectral}})
    ...
    for epoch in range(epochs):
        loss_metric.reset()
        for step, (batch, _) in enumerate(train_loader):#batch就是一个batch的X
            batch = batch.to(device)
            optimizer.zero_grad()
            cur_batch_size = batch.shape[0]
            t = torch.randint(0, diffusion.T , (cur_batch_size,), device=device).long()#生成随机整数0~diffusion.T
            t,_ = torch.sort(t)#升序排序
