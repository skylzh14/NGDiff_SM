...
class HSIDataLoader(object):
    def __init__(self, param={}) -> None:
        ...
        self.patch_size = self.data_param.get('patch_size', 32) # n * n   64
        self.padding = self.data_param.get('padding', True) # n * n
        self.remove_zeros = self.data_param.get('remove_zeros', False)
        self.batch_size = self.data_param.get('batch_size', 256)#256   20
    ...
    def load_data(self):
        ...
        elif self.data_sign == "512_512":
            data = hdf5storage.loadmat('../data/data.mat')['data']
            labels = sio.loadmat('../data/label.mat')['label']
        else:
            pass
    ...
    def custom_process(self, data):
        ...
        if self.data_sign == "512_512":
            h, w, spe = data.shape
            new_data = np.zeros((h,w,spe+7))
            new_data[:,:,:spe] = data#先把原数据赋给newdata
            new_data[:, :, spe:spe+7] = data[:,:,:7]
            return new_data
        return data


    def generate_torch_dataset(self, split=False, light_split=False):#light_split传参为True
        #1. 根据data_sign load data
        self.data, self.labels = self.load_data()
        input_maxs = []
        input_mins = []
        #1.1 norm化，按光谱维进行norm
        norm_data = np.zeros(self.data.shape)
        for i in range(self.data.shape[2]):
            input_max = np.max(self.data[:,:,i])
            input_min = np.min(self.data[:,:,i])
            #保存每个通道的最大值和最小值
            input_maxs.append(input_max)
            input_mins.append(input_min)
            norm_data[:,:,i] = (self.data[:,:,i]-input_min)/(input_max-input_min) * 2 - 1  # [-1,1]
        hdf5storage.savemat("xian_max&min.mat", {'maxs': np.array(input_maxs),
     'mins': np.array(input_mins)})
        print('[data] load data shape data=%s, label=%s' % (str(norm_data.shape), str(self.labels.shape)))
        self.data = norm_data

        #1.2 专门针对特殊的数据集补充或删减一些维度，对于polsar需要观察维度，一般为8的倍数。维度是9，则可扩为16维；维度是57，则可扩为64维。
