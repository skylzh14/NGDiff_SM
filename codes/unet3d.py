...
class Block3d(nn.Module):
    ...
    def forward(self, x, t, ):
        ...
        '''
        上述这一行代码用于在时间嵌入张量的末尾添加三个维度，以便与 h 的张量相加。
        (..., ) 表示保留 time_emb 张量中的所有现有维度。
        (None, ) * 3 表示添加三个额外的维度，每个维度都是大小为 1 的维度。
        '''
        # Add time channel
        h = h + time_emb
        # Second Conv
...
class SinusoidalPositionEmbeddings(nn.Module):#正余弦嵌入，将时间t嵌入
...
class SimpleUnet(nn.Module):
    ...
    def __init__(self, _image_channels):
        ...
        self.output = nn.Conv3d(up_channels[-1], image_channels, out_dim)#shape:[B,1,c,h,w]->[8,1,64,64,64]
    ...
    def forward(self, x, timestep, feature=False):
        ...
        for up in self.ups:
            residual_x = residual_inputs.pop()#弹出residual_inputs最后的元素
